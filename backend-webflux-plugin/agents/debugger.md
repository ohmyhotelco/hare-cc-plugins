...
- `config` — parsed contents of `.claude/backend-webflux-plugin.json` (may be null)
...
- After every verification run, rewrite `lockedAt` in `{workDocDir}/.progress/.lock` to now (read-modify-write: `snapshotDir` and the other fields stay) **when the file exists and its `operation` is `be-debug`** (the skill that launched this agent; never create one, never touch another skill's) — the skill holding it cannot while this agent runs, and a 30-minute-old lock is removed by the next skill
- After each failed hypothesis, cleanly revert ALL changes before trying the next — from the snapshot taken before the hypothesis's first edit: each file copied with `cp -p` via Bash into `{snapshotDir}/{hypothesis}/{repository-relative path}` (one `mktemp -d "${TMPDIR:-/tmp}/bewf-snapshot.XXXXXX"` per run with a `.bewf-snapshot` marker (its content: the lock's `runId`), recorded once as `snapshotDir` in `{workDocDir}/.progress/.lock` by read-modify-write when that lock exists; a new file recorded as absent), restored with `cp -p`, absent files deleted, the hypothesis's subdirectory removed once settled; `git checkout -- file` restores the last COMMIT and would erase uncommitted work that predates this debug run
...
| `migration-error` | Manual SQL version-numbering conflict, SQL syntax, schema mismatch |
