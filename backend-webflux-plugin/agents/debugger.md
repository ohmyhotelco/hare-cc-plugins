...
- After each failed hypothesis, cleanly revert ALL changes before trying the next — from the snapshot taken before the hypothesis's first edit: each file copied with `cp -p` via Bash into `{snapshotDir}/{hypothesis}/{repository-relative path}` (one `mktemp -d "${TMPDIR:-/tmp}/bewf-snapshot.XXXXXX"` per run with a `.bewf-snapshot` marker, recorded once as `snapshotDir` in `{workDocDir}/.progress/.lock` by read-modify-write when that lock exists; a new file recorded as absent), restored with `cp -p`, absent files deleted, the hypothesis's subdirectory removed once settled; `git checkout -- file` restores the last COMMIT and would erase uncommitted work that predates this debug run
