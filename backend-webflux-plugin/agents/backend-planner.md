...
5. If UI DSL is not available:
   ...
   - Infer field types from context clues (e.g., "email" → String with email validation, "created date" → LocalDateTime)
...
3. **Existing migrations** -- scan `src/main/resources/migration/` to determine the next migration version number
