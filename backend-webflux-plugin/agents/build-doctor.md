...
- **The same error persists, or the fix produced a new error in the same file/method
  ...
  step left behind. Revert means restoring the snapshot taken before the attempt's first edit: once per run create `mktemp -d "${TMPDIR:-/tmp}/bewf-snapshot.XXXXXX"` with a marker file `.bewf-snapshot` inside and record it as `snapshotDir` in `{workDocDir}/.progress/.lock` (be-build holds it, `operation: be-build`; read-modify-write; never another skill's lock); before each attempt copy every file you are about to touch with `cp -p` via Bash into `{snapshotDir}/{attempt}/{repository-relative path}` (a file that does not exist yet is recorded as absent), restore with `cp -p` from there, delete the files recorded absent, remove the attempt's subdirectory once settled; after every Gradle run rewrite `lockedAt` to now (read-modify-write) — four ten-minute builds outlast the 30-minute stale rule — never `git checkout -- file` or `git stash`, which restore the last COMMIT and on a tree full of uncommitted work erase everything written before this attempt; never a copy inside the repository, which the tree hash or a `git add -A` would pick up.
