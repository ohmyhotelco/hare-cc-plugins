...
- **The same error persists, or the fix produced a new error in the same file/method
  ...
  step left behind. Revert means restoring the snapshot taken before the attempt's first edit: copy each file you are about to touch into a `mktemp -d` directory outside the repository with `cp -p` via Bash (the Write tool is text-only; `-p` keeps mode and a symlink's target) and record its path as `snapshotDir` in `{workDocDir}/.progress/.lock` (be-build holds it, `operation: be-build`; never write into another skill's lock), so the skill can remove it after a crash (a file that does not exist yet is recorded as absent), restore with `cp -p` from there, delete the files recorded absent, and remove the directory when the run ends — never `git checkout -- file` or `git stash`, which restore the last COMMIT and on a tree full of uncommitted work erase everything written before this attempt; never a copy inside the repository, which the tree hash or a `git add -A` would pick up.
