...
| `enum` | `{EntityName}{FieldName}` (a Java enum — see Enum Handling) | `VARCHAR(50)` | `enumValues` lists the constants |
...
- `javaType` is a Java enum `{EntityName}{FieldName}` (e.g., `EmployeeStatus`) in `data/`, generated by `be-crud` from `entities[].fields[].enumValues` (constant names, `^[A-Z][A-Z0-9_]*$`)
- Column type: `VARCHAR(50)`, mapped by name with nothing extra: Spring Data R2DBC writes an enum as its `name()` and reads the `String` back through its conversion service; MyBatis's default `EnumTypeHandler` does the same — no converter, no `typeHandler`
