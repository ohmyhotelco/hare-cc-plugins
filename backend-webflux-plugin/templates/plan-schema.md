...
| `datetime` | `LocalDateTime` | `DATETIME(6)` | MySQL has no timezone-aware timestamp type; store UTC by application convention |
...
- Column type: `VARCHAR(50)`, mapped by name (R2DBC: manual `Enum.valueOf()`/`.name()` conversion via a converter; MyBatis: `typeHandler` in the mapper XML — neither profile has JPA's `@Enumerated(EnumType.STRING)`)
