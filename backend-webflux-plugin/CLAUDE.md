...
**Lock file**: Skills that modify progress files must acquire `{workDocDir}/.progress/.lock` before writing. Release on completion or failure. Stale locks (older than 30 minutes) are automatically removed — so a holder that runs longer refreshes `lockedAt`: the implement, review-fixer, debugger and build-doctor agents rewrite it after every Gradle run (the skill that took it is blocked on the agent), and staleness counts from the last refresh. Acquisition is check-then-write, not atomic: the lock guards against a forgotten or crashed run, not against two sessions racing on one project. **Release protocol** (every skill's release line and every stale-lock removal means this): read the lock first; delete it only if its `operation` is the releasing skill's own (a stale-lock removal is the one exception) — a lock another skill took after removing yours as stale is theirs: leave it and say so. Before deleting, remove the directory its `snapshotDir` names **only when** that path lies under `${TMPDIR:-/tmp}/`, matches `bewf-snapshot.*`, and contains the marker file `.bewf-snapshot` the agent wrote — a lock file is project-writable text, and an unvalidated path would be an `rm -rf` of anything it names; otherwise leave the directory and report it. An agent keeps one snapshot directory per run (`mktemp -d "${TMPDIR:-/tmp}/bewf-snapshot.XXXXXX"` + the marker), records it once as `snapshotDir` (read-modify-write, as is every `lockedAt` refresh — never a rewrite of the whole file), stores each file under its repository-relative path in a per-attempt subdirectory, and removes a subdirectory once its attempt is settled.
