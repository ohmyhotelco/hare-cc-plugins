...
**A status is only as good as the tree it was earned on.** `be-verify` records `scripts/source-tree-hash.sh` (src/, build and settings files, config/, gradle/) as `pipeline.verification.tree`; `be-review` and `be-jira-auto` recompute it and refuse a `verified`/`reviewed`/`done` status on a different tree — the code goes back through `be-verify`; `be-commit` compares the **staged** tree and asks before committing a mismatch, then marks the record `committed` so later tickets do not trip over it. Editing source after verification never demotes the status by itself; the hash is what makes the edit visible.
...
**Lock file**: Skills that modify progress files must acquire `{workDocDir}/.progress/.lock` before writing. Release on completion or failure. Stale locks (older than 30 minutes) are automatically removed — so a holder that runs longer refreshes `lockedAt`: the implement, review-fixer, debugger and build-doctor agents rewrite it after every Gradle run (the skill that took it is blocked on the agent), and staleness counts from the last refresh. Acquisition is check-then-write, not atomic: the lock guards against a forgotten or crashed run, not against two sessions racing on one project. **Release protocol** (every skill's release line and every stale-lock removal means this): read the lock first; delete it only if its `operation` **and `runId`** are the releasing run's own (a stale-lock removal is the one exception) — a lock another run took after removing yours as stale is theirs, even when it is the same skill: leave it and say so. Before deleting, remove the directory its `snapshotDir` names **only when** its canonical path (`cd … && pwd -P`, so a symlink or `..` cannot point elsewhere) lies under the canonical `${TMPDIR:-/tmp}/`, matches `bewf-snapshot.*`, and contains the marker file `.bewf-snapshot` whose content is this lock's `runId` — a lock file is project-writable text, and an unvalidated path would be an `rm -rf` of anything it names; otherwise leave the directory and report it. An agent keeps one snapshot directory per run (`mktemp -d "${TMPDIR:-/tmp}/bewf-snapshot.XXXXXX"`, the marker written with the lock's `runId`), records it once as `snapshotDir` (read-modify-write, as is every `lockedAt` refresh — never a rewrite of the whole file), stores each file under its repository-relative path in a per-attempt subdirectory, and removes a subdirectory once its attempt is settled.

Lock file format:
```json
{ "lockedAt": "{ISO 8601}", "operation": "be-code", "feature": "{feature}", "runId": "{random, per run}", "snapshotDir": "{optional: an agent's revert snapshot, outside the repository}" }
```
...
- (not a config key) the plugin's install directory is `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/plugins/data/backend-webflux-plugin/pluginRoot`, rewritten by the SessionStart hook at every session start/resume — a skill's Bash never sees `${CLAUDE_PLUGIN_ROOT}`; `be-verify`, `be-review`, `be-commit` and `be-jira-auto` run `scripts/source-tree-hash.sh` / `pre-commit-check.sh` from it and stop when the file is absent
- `buildCommand`: Full build command (default: `{gradleCommand} build`)
- `testCommand`: Test-only command (default: `./gradlew test`)
- `basePackage`: Root Java package (e.g., "com.example") — in a path such as `{sourceDir}/{basePackage}/` it stands for the package with `.` replaced by `/` (`src/main/java/com/example/`)
