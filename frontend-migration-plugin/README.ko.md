...
> 상태: 기능 완성 툴링(v0.17.3). 이 플러그인은 제품 앱을 포함하지 않으며, 마이그레이션
...
- **3중 패리티 게이트** — 생성 후 순서대로 통과해야 함: `fm-verify`(기술: 빌드/타입/단위테스트 +
  ESLint; Prettier는 advisory), `fm-e2e`(레거시처럼 동작하는가?), `fm-parity`(레거시처럼 보이고/계약/추적되는가?). 셋 다
  통과 전까지 라우트 플립 **차단**.
...
- **Codex 독립 감사** — 활성화 시(기본) 각 감사 대상 단계(analyze/plan/gen/verify/e2e/parity/route
  — fm-style-spec 제외)마다 **Codex**의 독립 제2 리뷰(어드바이저리)를 받아 `codex-audit.json`에 기록. 페이지 상태는 바꾸지 않으며, 유일한 소프트 게이트는
  `fm-route --flag-on`에서 미해결 high-severity 지적을 확인(ack)받는 것. Codex CLI 필요, 없으면 자동 skip.
...
## 외부 스킬 (frontend-react-plugin과 공유)

범용 React/테스트 지식은 여기서 다시 작성하지 않습니다 — `fm-init`이 `frontend-react-plugin`과
동일한 업스트림 스킬을 `npx skills add … --copy`로 설치(`.claude/skills/`에 vendoring)하여, 생성되는
React 코드가 조직 전체에서 일관되게 유지됩니다. 마이그레이션 고유 지식(Angular→React 매핑, Strangler
Fig, WebView/SSO)은 대신 `templates/`에 둡니다.

| 스킬 | 출처 | 용도 |
| --- | --- | --- |
| `react-router-framework-mode` | `remix-run/agent-skills` | RR v7 **framework 모드** 라우팅 (loader/action, per-route SSR/SSG/SPA) |
| `vitest` | `antfu/skills` | 단위/컴포넌트 테스트 패턴 |
| `vercel-react-best-practices` | `vercel-labs/agent-skills` | React 성능 — **SSR-aware**로 적용 (framework 모드, Vite SPA 아님) |
...
# 2. 페이지 루프
/frontend-migration-plugin:fm-style-spec hotel-booking-info   # → style-spec.json (라이브 레거시 computed 값 + 자산 + 구조)
...
| 게이트 | 스킬 | 검사 | 실패 시 |
| --- | --- | --- | --- |
| 1 · 기술 | `fm-verify` | build, `tsc`(composite 인식), Vitest, ESLint(하드); Prettier `--check`(advisory) | `fm-fix` (verify-fix) |
...
- **게이트가 실패했다.** `/frontend-migration-plugin:fm-fix <page>` — 최신 실패 리포트에서
  모드(verify/e2e/parity) 자동 감지, 최소 수정. 코드가 바뀌므로 모든 게이트 증거가 무효화되어
  페이지는 `generated`로 돌아가고, fm-verify부터 체인 전체를 다시 실행해 확인합니다.
...
- `templates/` — 매핑 카탈로그, 공유 패키지 스펙/규약, WebView 브리지, Hana SSO, Strangler Fig,
  TDD 규칙, migration-plan 스키마, 비주얼 패리티 체크리스트(`visual-parity-checklist.md` — 비주얼 게이트
  축 목록 + 크로스-프레임워크 픽셀 diff 대체 프로토콜), ESLint/Prettier 린트·포맷 게이트 설정(`eslint-config.md`, `prettier-config.md`)
