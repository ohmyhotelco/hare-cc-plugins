...
rule as Step 4), over the same watch-path union — axes 1–3 **plus `e2ePaths[]`** (CLAUDE.md →
Gate Result Accounting F). Step 4 compares against both: this gate
...
- `result: pass` → `apps[app].pages[page].status = "e2e-passed"`, and record
  ...
  **plus `e2ePaths[]`**; resolve `packagesDir` and `monorepoRoot` in Step 0 and read the plan's
  `sharedDeps[]` here. **First record the runner's `filesChanged[]` under `e2ePaths[]`**
  (Read-Modify-Write under `.tracker.lock`; add, never drop) — every file it created or modified:
  specs, page objects, fixtures, helpers alike, since any of them outside the watch union can be
  weakened later without moving the recorded tree. **Not into `sourcePaths[]`**: that is what
  `verify` stamped, and it ran before these files existed — merging them there made every first
  `--flag-on` stale and every re-run of this gate (which rewrites specs) re-stale it. The paths are
  **repo-relative** (the `sourcePaths` basis); one that does not start with `{appDir}` is the
  runner's mistake — resolve it against `appDir` before recording, or the hash watches a
  nonexistent root path. Then compute the record-time manifest/hash over the updated union. Compare with
