...
(missing → run `fm-plan {page}`; stop). Read `targetDir`, `appDir`, `packagesDir`, `routerMode`,
`workingLanguage`, and the plan's `buildOrder` + `blockers`.
...
- `foundation` → **foundation-generator** (types + MSW + harness; no TDD)
- `api` / `store` / `component` / `page` → **tdd-cycle-runner** (Red-Green per unit)
- `integration` → **integration-generator** (routes + i18n + MSW global)
