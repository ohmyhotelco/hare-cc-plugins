...
> **Ohmyhotel & Co** — Astro, shadcn/ui (또는 Figma 기반 커스텀 컴포넌트), SEO 최적화를 활용한 마케팅 홈페이지 개발을 위한 Claude Code 플러그인
...
| 컴포넌트 | shadcn/ui + Lucide icons (디자인 시스템 존재 시 Figma 기반 커스텀 컴포넌트로 자동 교체) |
...
2. /homepage-plugin:hp-design-sync                    # Figma 디자인 토큰 동기화 (선택사항)
3. /homepage-plugin:hp-plan                           # 대화형으로 페이지와 섹션 정의
4. /homepage-plugin:hp-gen                            # Astro 페이지 및 컴포넌트 생성
5. /homepage-plugin:hp-verify                         # 빌드 품질 검증 (선택사항)
6. /homepage-plugin:hp-review                         # SEO + 품질 코드 리뷰
7. /homepage-plugin:hp-fix {page}                     # 리뷰 이슈 수정 (해당 시)
```
...
**구문**: `/homepage-plugin:hp-design-sync [figma-file-url]`

**사용 시점**: `hp-init` 이후, Figma 디자인 파일이 있고 커스텀 컴포넌트 생성을 위한 디자인 토큰을 추출하려 할 때. 선택사항 — 이 스킬 없이도 `hp-gen`은 shadcn/ui 기본값을 사용합니다.

**사전 조건**: Figma MCP 서버가 연결되어 있어야 합니다 (원격 `https://mcp.figma.com/mcp` 또는 데스크탑 플러그인).

**동작 내용**:
1. URL 인자, 설정 파일, 또는 사용자 입력에서 Figma 파일 키를 확인
2. Figma MCP 연결 검증 (사용 불가 시 설정 방법 안내 후 종료)
3. 파일 구조 탐색 (페이지 기반 또는 라이브러리 기반)
4. design-token-extractor 에이전트를 실행하여 다음을 추출:
   - 색상 토큰, 타이포그래피, 간격, 테두리 반경, 그림자
   - shadcn/ui 동등 항목에 매핑된 컴포넌트 정의
5. `docs/design-system/design-tokens.json` 및 `docs/design-system/component-map.json` 작성
6. `figmaFileKey` 및 `figmaFileUrl`을 포함하여 `.claude/homepage-plugin.json` 업데이트
7. 출력 검증 및 요약 표시

**재실행 지원**: 이후 실행 시 `replace` (새로 추출) 또는 `update` (기존과 병합) 모드를 선택할 수 있습니다.
...
Figma 디자인 파일이 있다면 디자인 토큰과 컴포넌트 정의를 추출합니다. 이를 통해 `hp-gen`이 shadcn/ui 기본값 대신 Figma 기반 커스텀 컴포넌트를 생성할 수 있습니다. Figma MCP 서버 연결이 필요합니다.
...
**역할**: Figma MCP → 디자인 토큰 + 컴포넌트 맵 (`design-tokens.json`, `component-map.json`).

MCP를 통해 Figma 파일에 연결하여 디자인 토큰(색상, 타이포그래피, 간격, 그림자)과 컴포넌트 정의를 추출합니다. 페이지 기반 파일(Figma 페이지가 웹사이트 페이지를 나타내는 경우)과 라이브러리 기반 파일(디자인 시스템 라이브러리) 모두 지원합니다. `hp-gen`이 커스텀 컴포넌트 생성을 위해 읽는 JSON 파일을 출력합니다. Opus 모델을 사용합니다.
...
| Design Sync | `/homepage-plugin:hp-design-sync` | Figma 디자인 토큰 추출 (선택사항, Figma MCP 필요) |
...
| `figmaFileKey` | (선택사항) 디자인 토큰 추출을 위한 Figma 파일 키 | — |
| `figmaFileUrl` | (선택사항) 참조용 전체 Figma 파일 URL | — |
...
│   ├── ui/                         ← shadcn/ui 또는 Figma 기반 커스텀 컴포넌트
...
`docs/design-system/` 하위의 디자인 시스템 파일 (`hp-design-sync`로 생성):

| 파일 | 용도 |
|------|---------|
| `design-tokens.json` | Figma에서 추출한 디자인 토큰 (색상, 타이포그래피, 간격, 그림자) |
| `component-map.json` | Figma 컴포넌트-코드 매핑 (shadcn/ui 동등 항목) |
...
- [x] Figma MCP 통합 (hp-design-sync를 통한 자동 디자인 동기화)
