#!/usr/bin/env bash
# Gate-evidence content hash over a feature's watch paths.
#
# Ported verbatim from frontend-migration-plugin (the hashing logic is generic; only this header
# and the watch-path definition below are plugin-specific). Do not "simplify" the body — every
# branch in it closes a false-pass defect found in that plugin's audit rounds.
#
# WHY THIS IS A SCRIPT
# --------------------
# `gateEvidence.{gate}.tree` is only meaningful if the producer (fe-verify / fe-review / fe-e2e)
# and the consumer (the next gate, fe-progress, the session hook) compute it the SAME way.
# A shell pipeline copied into five call sites drifts; one executable cannot.
#
# Three rules — every past defect broke one of them:
...
#   on PATH can make it anything. The defences here are against ordinary settings that differ
#   between a producer and a consumer — locale, working directory, sparse checkouts — not against
#   a hostile environment, which no part of this script could survive.
#
# USAGE
#   gate-tree-hash.sh [--manifest] [--exclude <repo-relative-path>]... [--] <watch-path>...
#
#   <watch-path>  repo-relative paths — the two axes (CLAUDE.md -> Gate Evidence & Freshness):
#                 (1) the feature's `implementation.sourcePaths[]` from its progress file, and
#                 (2) the feature's own `plan.json`. Resolved from the repo root regardless of
#                 the caller's working directory, and matched LITERALLY.
...
"*) echo "gate-tree-hash: path contains a newline, cannot record: $f" >&2; exit 1 ;;
  ...
  if [ -L "$f" ]; then
    ...
    # path of a gate's freshness check. A watch path cannot produce a symlink anyway:
    # `sourcePaths[]` are the files fe-gen wrote, and pnpm's `node_modules` link forest is
    ...
    echo "  or keep symlinks out of sourcePaths[]." >&2
    exit 1
  elif [ -f "$f" ]; then
