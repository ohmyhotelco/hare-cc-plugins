...
1. Khởi chạy tác tử Debugger với phương pháp 4 giai đoạn:
   - **Root Cause Investigation**: Phân tích lỗi, truy vết đường dẫn mã, so sánh với đặc tả/kế hoạch
   - **Pattern Analysis**: Tìm kiếm lỗi cùng pattern, phân loại loại vấn đề (generation-bug, plan-bug, spec-bug, environment)
   - **Hypothesis Testing**: Lập tối đa 3 giả thuyết và kiểm tra tuần tự (3-strike leo thang)
   - **Implementation**: Áp dụng sửa lỗi TDD tối thiểu với xác minh
2. Nếu cả 3 giả thuyết đều thất bại, leo thang với phân tích cấu trúc và khuyến nghị
...
**Vai trò**: Kiểm tra lỗ hổng bảo mật frontend.

Quét vector XSS (dangerouslySetInnerHTML, eval, innerHTML), vấn đề lưu trữ token xác thực (localStorage), lộ bí mật (API key hardcode), vấn đề an toàn dữ liệu (ghi log PII ra console, open redirect) và lo ngại cấu hình (CSP, CORS, source map). Xuất báo cáo văn bản với đánh giá mức rủi ro.

### Test Reviewer

**Vai trò**: Kiểm tra chất lượng test (7 chiều).

Đánh giá chất lượng assertion, best practice Testing Library, pattern async, cấu trúc test, phân tích coverage, timing gate (tùy chọn — đo thời gian thực thi test thực tế qua vitest) và anti-pattern. Xuất báo cáo văn bản với điểm từng chiều.
...
Root Cause Investigation → Pattern Analysis → Hypothesis Testing (giới hạn 3-strike) → Implementation. Phân loại vấn đề theo loại (generation-bug, plan-bug, spec-bug, environment). Leo thang sau 3 giả thuyết thất bại với phân tích cấu trúc.
...
### Skill kiểm tra Standalone

Các skill kiểm tra độc lập chạy ngoài pipeline. Không cần theo dõi tiến trình, lock file hoặc context tính năng.
...
Các skill này có thể chạy bất cứ lúc nào trên bất kỳ mã nào, không chỉ các tính năng được sinh bởi pipeline.
...
- **Trạng thái pipeline**: Quét tất cả tính năng và báo cáo trạng thái hiện tại với hướng dẫn bước tiếp theo:
  ...
  - `verified` → đề xuất `fe-review`
  - `verify-failed` → đề xuất `fe-debug`
  - `reviewed` → đề xuất `fe-fix` (cảnh báo) hoặc `fe-e2e` (nhận biết E2E)
  ...
  - `resolved` → đề xuất `fe-verify` hoặc `fe-review`
  - `escalated` → cảnh báo cần can thiệp thủ công
  - `done` → đề xuất `fe-e2e` nếu chưa chạy E2E, nếu không thì báo cáo hoàn thành (nhận biết E2E)
...
Các skill cấp tính năng (fe-plan, fe-gen, fe-verify, fe-review, fe-fix, fe-e2e, fe-debug, fe-progress) đọc `workingLanguage` từ tệp tiến độ. Tất cả đầu ra cho người dùng (tóm tắt, câu hỏi, phản hồi, hướng dẫn bước tiếp theo) được viết bằng ngôn ngữ làm việc.
