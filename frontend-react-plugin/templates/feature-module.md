...
> **Profile/mode variants (OTA Phase 1).** The sections below are the **admin default** (declarative/data
> SPA, Axios+Zustand, native forms, feature `routes.tsx`). When the config selects otherwise, apply the
> variant instead of the default:
> - `serverState == tanstack-query` → add `api/queries.ts` (§ Server-State Queries); Zustand holds UI/client state only. Full layering: `server-state.md`.
> - `formStack == rhf-zod` → the RHF + zod form variant (§ Form Component) instead of the native example.
> - `routerMode == framework` → feature `routes.ts` (RouteConfig fragment) + route-module pages that delegate to a page-body component (§ Framework-Mode Routing & Pages); shell files come from `framework-app-shell.md`.
> The variants are additive — an admin-default config generates exactly what it did before.
...
> the base client. `VITE_*` is client-public; server-only config uses non-`VITE_` `process.env`.

### Server-State Queries (`api/queries.ts`) — `serverState == tanstack-query` only

Query-key factory + `queryOptions`/`infiniteQueryOptions` + hooks, shared by loaders and components. The
factory is **fetch-client-agnostic** (takes the client as an argument) so it composes with the D13 split.
Full contract (staleTime, initialDataUpdatedAt, invalidation, anti-patterns): `server-state.md`.

```typescript
import { queryOptions } from '@tanstack/react-query';
import type { ApiClient } from '@/lib/api.types';
import type { Entity, ListParams, ListResponse } from '../types/{entity}';

export const entityKeys = {
  all: ['entities'] as const,
  list: (params?: ListParams) => [...entityKeys.all, 'list', params] as const,
  detail: (id: string) => [...entityKeys.all, 'detail', id] as const,
};

export function entityListQuery(client: ApiClient, params?: ListParams) {
  return queryOptions({
    queryKey: entityKeys.list(params),
    queryFn: () => client.get<ListResponse<Entity>>('/api/v1/entities', { params }),
    staleTime: 60_000, // loader-fed queries must set staleTime > 0 — see server-state.md rule (b)
  });
}
// mutations: useMutation + qc.invalidateQueries({ queryKey: entityKeys.all })
```

> When `serverState == tanstack-query`, server data lives in the query cache — **not** in the Zustand
> store. The store below is then trimmed to UI/client state only (search-form input, filters-as-UI,
> selected-id, dialogs); drop `list`/`total`/`loading`/`error` of server origin.
...
> **`formStack == rhf-zod` variant** (ota default): use react-hook-form + zod instead of the native form
> below. The zod schema lives in `schemas/{entity}Schema.ts` (generated by `foundation-generator` from the
> plan's validation rules); the form wires it with `zodResolver`, uses shadcn Form primitives, and renders
> `t()`-translated messages. Tests drive validation through `userEvent` and assert the rendered messages —
...
>           <FormItem>
>             <FormLabel>{t('entityForm.name.label')}</FormLabel>
>             <FormControl><Input {...field} /></FormControl>
>             <FormMessage />
>           </FormItem>
>         )} />
>         <Button type="submit" disabled={loading}>{loading ? t('actions.submitting') : t('actions.submit')}</Button>
>       </form>
>     </Form>
>   );
> }
...
#### Framework Mode (`routes.ts`)

Feature fragment is a `RouteConfig[]` built with `route()`/`index()`/`layout()`/`prefix()` from
`@react-router/dev/routes`. Route-module `file` values are **app-root-relative** (never fragment-relative
— RR resolves split configs against the app root). Auth/permission wrapping lives **inside** the route
module (`ProtectedRoute`/`RoleRoute` in the module component), not in the config. The central
`{baseDir}/routes.ts` imports and spreads this fragment (integration-generator).

```typescript
import { type RouteConfig, route } from '@react-router/dev/routes';

// paths are app-root-relative (from {baseDir})
export const entityManagementRoutes: RouteConfig = [
  route('entities', 'features/entity-management/routes/list.tsx'),
  route('entities/new', 'features/entity-management/routes/create.tsx'),
  route('entities/:id', 'features/entity-management/routes/detail.tsx'),
  route('entities/:id/edit', 'features/entity-management/routes/edit.tsx'),
];
```

Each `routes/*.tsx` is a **thin route module** (loader/`clientLoader`, `meta`, `ErrorBoundary`,
`HydrateFallback`, default export) that delegates to a page-body component under `pages/` — see
§ Framework-Mode Routing & Pages and `framework-app-shell.md`. SSG entries (parameterless, loader-less
only — D5) are added to the `prerender` array in `{appDir}/react-router.config.ts` by
integration-generator.

### Framework-Mode Routing & Pages

Per D10 the page splits into a thin route module (wrapper) and a tested page-body component:
- **Route module** (`routes/{name}.tsx`): exports `loader`/`clientLoader` (SSR/SSG fetch via the D13
  base client, returns `{ data, fetchedAt }`), `meta` (title/description keys translated server-side),
  `ErrorBoundary` (D10 error state), `HydrateFallback` (D10 loading state), and a default export that
...
> **Framework mode (D10)**: the TDD target is the extracted **page-body component**
> (`pages/{Entity}...Page.tsx`), rendered with plain Testing Library and given loader data as props — no
> `MemoryRouter` wrapper unless the body uses router hooks (then use `createRoutesStub`, never the route
> module directly). The route module (loader/meta/ErrorBoundary/HydrateFallback) is covered by
> `react-router typegen` + build + the Playwright E2E smoke, not this unit test. The example below is the
> admin/library-mode pattern.
