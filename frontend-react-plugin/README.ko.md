...
단일 책임, 일관된 패턴, 하드코딩된 문자열 없음, 오류 처리, TypeScript 엄격성, 컨벤션 준수, 아키텍처를 평가합니다. 파이프라인 모드(fe-review에서 Spec Review 통과 후 호출)와 standalone 모드(fe-clean-code에서 ad-hoc 감사 시 호출)를 지원합니다.
...
| Clean Code | `/frontend-react-plugin:fe-clean-code` | 클린 코드 감사 (7개 품질 차원 — quality-reviewer의 standalone 모드) |
