...
4. `.claude/frontend-react-plugin.json`을 생성합니다
5. 6개의 외부 스킬(React Router, Vitest, React Best Practices, Composition Patterns, Web Design Guidelines, Agent Browser)을 설치합니다
6. 다음 단계 옵션(planning-plugin 유무에 따른)을 표시합니다
...
3. **자동 감지**: 명세서가 없고 `--standalone`이 지정되지 않은 경우, 독립 실행 모드와 명세서 작성 중 선택을 안내합니다
4. Implementation Planner 에이전트를 실행하여 `plan.json`을 생성합니다
5. 계획 요약(파일, TDD 단계, shadcn/ui 의존성)을 표시합니다
6. 진행 파일을 `planned` 상태로 업데이트합니다
...
1. 4단계 방법론으로 Debugger 에이전트를 실행합니다:
   - **Observe**: 증거 수집 (오류 메시지, 로그, 재현 단계)
   - **Hypothesize**: 신뢰도 점수와 함께 순위화된 가설 수립
   - **Test**: 각 가설을 체계적으로 검증
   - **Fix**: TDD 수정 적용 및 검증
2. 해결 불가능한 경우, 상세 진단과 함께 에스컬레이션합니다
...
Observe → Hypothesize → Test → Fix. 신뢰도 점수가 포함된 순위화된 가설 목록을 유지합니다. 해결 불가능한 경우 에스컬레이션합니다.
...
- **잠금 메커니즘**: 상태 파일을 수정하는 스킬은 시작 전에 `.lock`을 획득합니다. 동일 기능에 대한 fe-gen/fe-fix/fe-review의 동시 실행을 방지합니다. 만료된 잠금(30분 초과)은 자동으로 제거됩니다.
...
- **파이프라인 상태**: 모든 기능을 스캔하고 현재 상태와 다음 단계 안내를 보고합니다:
  ...
  - `done` → E2E 미실행 시 `fe-e2e`를 제안하고, 그 외에는 완료를 보고합니다
...
기능 수준 스킬(fe-plan, fe-gen, fe-verify, fe-review, fe-fix, fe-debug)은 진행 파일에서 `workingLanguage`를 읽습니다. 모든 사용자 대면 출력(요약, 질문, 피드백, 다음 단계 안내)은 작업 언어로 제공됩니다.
