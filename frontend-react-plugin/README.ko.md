...
4. **Stage 2 — Quality Review** (spec review 통과 시에만): Quality Reviewer 에이전트가 단일 책임, 일관된 패턴, 하드코딩된 문자열 없음, 오류 처리, TypeScript 엄격성, 컨벤션 준수, 아키텍처, 단순성/과잉 설계(8개 차원, 1-10점)를 검사합니다
...
단일 책임, 일관된 패턴, 하드코딩된 문자열 없음, 오류 처리, TypeScript 엄격성, 컨벤션 준수, 아키텍처, 단순성/과잉 설계(죽은 코드, 표준 라이브러리 재구현, 구현이 하나뿐인 추상화)를 평가합니다. 파이프라인 모드(fe-review에서 Spec Review 통과 후 호출)와 standalone 모드(fe-clean-code에서 ad-hoc 감사 시 호출)를 지원합니다.

### Security Auditor

**역할**: 프론트엔드 보안 취약점 감사.

XSS 벡터(dangerouslySetInnerHTML, eval, innerHTML), 인증 토큰 저장 이슈(localStorage), 비밀 노출(하드코딩된 API 키), 데이터 안전성 이슈(콘솔 PII 로깅, 오픈 리다이렉트), 설정 관련 사항(CSP, CORS, 소스맵)을 스캔합니다. 위험 수준 평가를 포함한 텍스트 리포트를 출력합니다.

### Test Reviewer

**역할**: 테스트 품질 감사 (7개 차원).

Assertion 품질, Testing Library 모범 사례, 비동기 패턴, 테스트 구조, 커버리지 분석, 타이밍 게이트(선택 사항 — vitest로 실제 테스트 실행 시간 측정), 안티 패턴을 평가합니다. 차원별 점수가 포함된 텍스트 리포트를 출력합니다.
...
파이프라인과 독립적으로 실행되는 감사 스킬입니다. progress 추적, lock 파일, feature 컨텍스트가 필요 없습니다.

| Skill | Command | 설명 |
|-------|---------|------|
| Security | `/frontend-react-plugin:fe-security` | 보안 취약점 감사 (XSS, 인증 토큰, 비밀 키, 클라이언트 측 데이터 안전성) |
| Clean Code | `/frontend-react-plugin:fe-clean-code` | 클린 코드 감사 (8개 품질 차원 — quality-reviewer의 standalone 모드) |
| Test Review | `/frontend-react-plugin:fe-test-review` | 테스트 품질 감사 (assertion, Testing Library, 비동기 패턴, 커버리지, 타이밍 게이트) |

사용법: `fe-security [경로]`, `fe-clean-code [경로]`, `fe-test-review [테스트 경로]`

이 스킬들은 파이프라인에서 생성한 코드뿐만 아니라 어떤 코드에도 언제든지 실행할 수 있습니다.
