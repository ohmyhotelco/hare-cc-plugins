...
4. **Existing code** — read files generated by previous phases:
   ...
   - Mocks: `src/features/{feature}/mocks/*.ts` (factories, fixtures, handlers — always generated by foundation-generator regardless of `mockFirst`)
...
1. **Create stub file** for the implementation target (if not already present):
   ...
   For `store-tdd`:
   ```typescript
   // Stub: src/features/{feature}/stores/entityStore.ts
   export const useEntityStore = {} as Record<string, never>;
   ```

   For `component-tdd`:
   ```tsx
   // Stub: src/features/{feature}/components/EntityForm.tsx
   export function EntityForm() { return null; }
   ```

   For `page-tdd`:
   ```tsx
   // Stub: src/features/{feature}/pages/EntityListPage.tsx
...
2. **Write test file** based on plan's test cases:
   - Location: `src/features/{feature}/__tests__/{target}.test.{ts,tsx}`
...
```bash
npx vitest run src/features/{feature}/__tests__/{testFile} --reporter=verbose 2>&1
```
...
```json
{
  ...
  "red": {
    ...
    "stubFiles": ["src/features/{feature}/api/entityApi.ts"],
    "verifyResult": "fail",
    "failureCount": 5,
