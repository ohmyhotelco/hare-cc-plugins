...
1. **Plan** — read `planFile` → load `routes`, `i18n`, `mocks.globalSetupNeeded`, `mocks.globalFiles`
...
npx eslint {baseDir}/features/{feature}/ 2>&1

# If .eslintrc* exists (legacy config, ESLint v8):
npx eslint {baseDir}/features/{feature}/ --ext .ts,.tsx 2>&1
```
...
```json
{
  ...
  "filesCreated": [
    ...
    "{baseDir}/locales/ko/{feature}.json",
    "{baseDir}/locales/en/{feature}.json",
    "{baseDir}/locales/ja/{feature}.json",
    "{baseDir}/locales/vi/{feature}.json"
  ],
  "routeIntegration": {
