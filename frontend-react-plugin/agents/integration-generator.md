...
3. **Generated code** — read all files generated by previous phases:
   - Pages: `src/features/{feature}/pages/*.tsx` → identify page exports
...
Generate `src/features/{feature}/index.ts`:
```typescript
...
**c. Full test suite:**
```bash
npx vitest run src/features/{feature}/ --reporter=verbose 2>&1
```
...
```json
{
  ...
  "msw": {
    "globalSetup": true,
    "handlersUpdated": "src/mocks/handlers.ts"
  },
  "verification": {
    "tsc": "pass | fail",
...
- [ ] Aggregator pattern: `src/mocks/handlers.ts` imports all feature handlers
