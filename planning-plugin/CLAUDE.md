...
- Translation confirmation: before launching parallel translator agents during finalization, both the spec and review skills ask the user whether to translate now or skip (and run `/planning-plugin:translate` later). The review skill also offers a cancel option to return to review
...
- Bundle staleness: `validate-spec-format.sh` auto-transitions `bundleStatus` from `"current"` → `"stale"` when prototype source files (`prototypes/{feature}/src/`) are edited. `session-init.sh` warns on stale bundles. `/planning-plugin:bundle {feature}` rebuilds and restores `"current"`
