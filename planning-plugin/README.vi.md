...
3. Tác tử Prototype Generator đọc UI DSL (và đầu ra wireframe Stitch nếu có) và tạo dự án Vite + React 19 + TypeScript + TailwindCSS + shadcn/ui + React Router v7 + Lucide độc lập trong `src/prototypes/{feature}/`
...
**Quy trình thực hiện**:
1. Xác minh rằng prototype tồn tại tại `src/prototypes/{feature}/`
...
Đọc UI DSL JSON và tạo dự án Vite + React 19 + TypeScript + TailwindCSS + shadcn/ui + React Router v7 + Lucide hoàn chỉnh trong `src/prototypes/{feature}/`, sau đó đóng gói thành một tệp `bundle.html` độc lập duy nhất (có thể mở qua `file://`). Bao gồm dữ liệu mock, routing trang và tất cả thành phần shadcn/ui được tham chiếu. Prototype là độc lập — không phụ thuộc vào dự án chính. Sử dụng mô hình Opus.
