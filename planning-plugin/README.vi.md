...
> **Lưu ý**: Stitch MCP chỉ cần cho Stage 2 của `/planning-plugin:design`. Nếu không cấu hình, stage này tự động bị bỏ qua và pipeline tiếp tục mà không có wireframe.
...
```
/planning-plugin:design _shared                         # tạo DSL layout chia sẻ (+ Stitch tùy chọn)
/planning-plugin:design social-login                    # DSL tính năng tự động tham chiếu layout chia sẻ
/planning-plugin:prototype social-login                 # prototype sao chép component layout chia sẻ cục bộ
```
...
**Tự động tạo layout chia sẻ**: Khi chạy `/planning-plugin:spec`, plugin tự động phát hiện pattern layout chia sẻ từ câu trả lời luồng người dùng của Analyst. Nếu các màn hình chia sẻ sidebar + header shell cố định:
...
- Nếu `_shared/en/screens.md` chưa tồn tại: bạn được hỏi có muốn tạo hay không. Nếu đồng ý, một màn hình layout chia sẻ (với component `Slot`) được tạo và directive `@layout:` của tính năng được kích hoạt. Bước hoàn thiện sau đó hướng dẫn bạn chạy `design _shared` trước `design {feature}`.
...
**Cú pháp**: `/planning-plugin:sync-stitch feature-name [--screen=screen-id]`

**Khi nào sử dụng**: Sau khi chỉnh sửa wireframe trên trang web Stitch, để lấy HTML/PNG/design token đã cập nhật về dự án cục bộ. Khác với `design --stage=stitch` tạo lại wireframe từ DSL, `sync-stitch` lấy lại nội dung từ các màn hình Stitch hiện có.
...
| Tình huống | Lệnh |
|------------|-------|
| Đã chỉnh sửa wireframe trên trang web Stitch | `/planning-plugin:sync-stitch {feature}` |
| UI DSL đã thay đổi (thay đổi màn hình spec) | `/planning-plugin:design {feature} --stage=stitch` |
| Chỉ cần đồng bộ một màn hình | `/planning-plugin:sync-stitch {feature} --screen=screen-id` |
...
**Cú pháp**: `/planning-plugin:bundle feature-name`
...
- Tự động tạo layout chia sẻ: Skill `spec` tự động phát hiện pattern layout chia sẻ và tùy chọn tạo `_shared/en/screens.md` trong quá trình tạo bản nháp (Step 3.5). Luôn tạo trong thư mục `en/` bất kể `workingLanguage`
