...
- **Wireframe Stitch lỗi thời**: Cảnh báo nếu tệp UI DSL được chỉnh sửa sau lần tạo wireframe Stitch cuối cùng (`stitch.status: "stale"`)
...
- **Phát hiện Stitch lỗi thời**: Nếu tệp UI DSL trong `docs/specs/{feature}/ui-dsl/` được chỉnh sửa và `stitch.status` là `"completed"`, tự động chuyển sang `"stale"`
