<!-- Synced with en version: 2026-03-12T00:00:00Z -->
...
- **Stitch Wireframe** — Tạo wireframe trực quan từ UI DSL qua Google Stitch MCP
- **Prototype Generator** — Tạo prototype React độc lập từ UI DSL (+ wireframe Stitch)
...
| Máy chủ | Sử dụng bởi |
|----------|-------------|
| `notion` | Tác tử Notion Syncer (`/planning-plugin:pp-sync-notion`) |
...
Máy chủ Stitch MCP **không được đóng gói** trong `plugin.json` vì yêu cầu header API key. Bạn cần cấu hình thủ công:

1. Lấy Google AI API key từ [Google AI Studio](https://aistudio.google.com/apikey)
2. Thêm máy chủ Stitch MCP:
   ```
   claude mcp add stitch --transport http https://stitch.googleapis.com/mcp --header "X-Goog-Api-Key: YOUR_API_KEY" -s user
   ```
3. Xác minh thiết lập bằng cách chạy `/mcp` — `stitch` sẽ xuất hiện trong danh sách máy chủ

> **Lưu ý**: Stitch MCP chỉ cần cho Stage 2 của `/planning-plugin:pp-design`. Nếu không cấu hình, stage này tự động bị bỏ qua và pipeline tiếp tục mà không có wireframe.

**Xác thực qua OAuth**:
1. Chạy `/mcp` trong Claude Code
2. Chọn máy chủ (`notion`)
...
Thiết lập `.claude/planning-plugin.json` với ngôn ngữ làm việc, ngôn ngữ được hỗ trợ và URL Notion tùy chọn. Bước này là bắt buộc trước khi chạy `/planning-plugin:pp-spec` — skill pp-spec đọc cấu hình từ tệp này.
...
**Cú pháp**: `/planning-plugin:pp-init`
...
**Cú pháp**: `/planning-plugin:pp-spec "feature description"`
...
4. Nếu các màn hình của tính năng chia sẻ sidebar + header shell: phát hiện pattern layout chia sẻ và tùy chọn tạo `docs/specs/_shared/en/screens.md` (xem [Layout chia sẻ](#layout-chia-sẻ-_shared))
5. Bản dịch sang các ngôn ngữ được hỗ trợ khác được tạo song song
6. Planner và Tester thực hiện đánh giá tuần tự với chấm điểm
7. Bạn xử lý phản hồi, bản dịch được đồng bộ, lặp lại hoặc hoàn thiện
...
**Cú pháp**: `/planning-plugin:pp-review feature-name`
...
5. Bạn xử lý vấn đề; nếu chọn hoàn thiện, bản dịch được đồng bộ trước khi hoàn thiện
...
**Cú pháp**: `/planning-plugin:pp-translate feature-name [--file=<name>]`
...
5. Nếu `notionParentPageUrl` đã được cấu hình, tự động đồng bộ các ngôn ngữ đã dịch lên Notion
6. Báo cáo các đánh dấu `<!-- NEEDS_REVIEW -->` mà Translator để lại cho nội dung không rõ ràng

**Ví dụ**:
```
/planning-plugin:pp-translate social-login                    # đồng bộ toàn bộ (tất cả tệp)
/planning-plugin:pp-translate social-login --file=screens       # chỉ đồng bộ screens.md
```
...
**Cú pháp**: `/planning-plugin:pp-progress [feature-name]`
...
Phần Design Status, Design System và Notion Sync chỉ hiển thị khi trường tương ứng tồn tại trong tệp tiến độ.
...
**Cú pháp**: `/planning-plugin:pp-migrate-language feature-name --to=vi`
...
**Cú pháp**: `/planning-plugin:pp-sync-notion feature-name [--lang=xx]`
...
**Cú pháp**: `/planning-plugin:pp-design feature-name [--stage=dsl|stitch]`
...
**Ví dụ**:
```
/planning-plugin:pp-design social-login                    # toàn bộ pipeline (stage 1→2 + review gate)
/planning-plugin:pp-design social-login --stage=dsl        # chỉ tạo DSL
/planning-plugin:pp-design social-login --stage=stitch     # chỉ tạo wireframe Stitch
```

> **Lưu ý**: Stage 2 (Stitch) là tùy chọn. Stitch yêu cầu `claude mcp add stitch --transport http https://stitch.googleapis.com/mcp --header "X-Goog-Api-Key: <key>" -s user`. Để tạo prototype, chạy `/planning-plugin:pp-prototype {feature}` sau khi xem xét wireframe.

**Layout chia sẻ (`_shared`)**: Sử dụng `_shared` làm tên feature để tạo DSL cho các màn hình layout toàn ứng dụng (sidebar + header shell) được chia sẻ giữa nhiều tính năng. Các tính năng tham chiếu layout chia sẻ qua directive `<!-- @layout: _shared/main-layout -->` trong `screens.md`. Quy trình khuyến nghị:

```
/planning-plugin:pp-design _shared                         # tạo DSL layout chia sẻ (+ Stitch tùy chọn)
/planning-plugin:pp-design social-login                    # DSL tính năng tự động tham chiếu layout chia sẻ
/planning-plugin:pp-prototype social-login                 # prototype sao chép component layout chia sẻ cục bộ
```

> `_shared` bỏ qua xác thực vòng đời spec (không cần tệp progress) — chỉ cần `docs/specs/_shared/en/screens.md` tồn tại. Các tính năng không có directive `@layout:` sử dụng phát hiện layout cục bộ như trước (tương thích ngược).

**Tự động tạo layout chia sẻ**: Khi chạy `/planning-plugin:pp-spec`, plugin tự động phát hiện pattern layout chia sẻ từ câu trả lời luồng người dùng của Analyst. Nếu các màn hình chia sẻ sidebar + header shell cố định:
- Nếu `_shared/en/screens.md` đã tồn tại: directive `@layout:` được kích hoạt tự động, và các component shell được loại bỏ khỏi định nghĩa màn hình của tính năng.
- Nếu `_shared/en/screens.md` chưa tồn tại: bạn được hỏi có muốn tạo hay không. Nếu đồng ý, một màn hình layout chia sẻ (với component `Slot`) được tạo và directive `@layout:` của tính năng được kích hoạt. Bước hoàn thiện sau đó hướng dẫn bạn chạy `pp-design _shared` trước `pp-design {feature}`.
- Nếu không phát hiện pattern hoặc bạn từ chối: tính năng sử dụng phát hiện layout cục bộ như trước.

---

### `/planning-plugin:pp-prototype`

**Cú pháp**: `/planning-plugin:pp-prototype feature-name`

**Khi nào sử dụng**: Sau khi xem xét wireframe Stitch (hoặc sau khi tạo DSL nếu bỏ qua Stitch), để tạo prototype React.

**Quy trình thực hiện**:
1. Nếu UI DSL của tính năng tham chiếu layout chia sẻ (`source: "_shared"`), xác minh rằng `docs/specs/_shared/ui-dsl/manifest.json` tồn tại
2. Nếu wireframe Stitch tồn tại, hỏi liệu có muốn đồng bộ các thay đổi mới nhất từ Stitch trước khi tạo
3. Tác tử Prototype Generator đọc UI DSL (và đầu ra wireframe Stitch nếu có) và tạo dự án Vite + React 19 + TypeScript + TailwindCSS + shadcn/ui + React Router v7 + Lucide độc lập trong `prototypes/{feature}/`
4. Đóng gói thành một tệp `bundle.html` độc lập duy nhất (có thể mở qua `file://`)

> Các component layout chia sẻ được sao chép cục bộ vào prototype (copy-on-reference) để giữ prototype tự chứa.
...
**Cú pháp**: `/planning-plugin:pp-sync-stitch feature-name [--screen=screen-id]`

**Khi nào sử dụng**: Sau khi chỉnh sửa wireframe trên trang web Stitch, để lấy HTML/PNG/design token đã cập nhật về dự án cục bộ. Khác với `pp-design --stage=stitch` tạo lại wireframe từ DSL, `pp-sync-stitch` lấy lại nội dung từ các màn hình Stitch hiện có.

**Quy trình thực hiện**:
1. Đọc `stitch-manifest.json` hiện có để tìm dự án Stitch và ánh xạ màn hình
2. So sánh màn hình Stitch với manifest cục bộ (báo cáo màn hình mới/đã xóa)
3. Lấy lại mã HTML và ảnh chụp PNG cho mỗi màn hình khớp
4. Tạo lại `DESIGN.md` (token front-matter + prose) và `shadcn-mapping.json` từ nội dung đã cập nhật
5. Đánh dấu bundle prototype là `"stale"` nếu prototype tồn tại

**Khi nào sử dụng lệnh nào**:
| Tình huống | Lệnh |
|------------|-------|
| Đã chỉnh sửa wireframe trên trang web Stitch | `/planning-plugin:pp-sync-stitch {feature}` |
| UI DSL đã thay đổi (thay đổi màn hình spec) | `/planning-plugin:pp-design {feature} --stage=stitch` |
| Chỉ cần đồng bộ một màn hình | `/planning-plugin:pp-sync-stitch {feature} --screen=screen-id` |
...
**Cú pháp**: `/planning-plugin:pp-bundle feature-name`

**Khi nào sử dụng**: Khi `bundleStatus` là `"stale"` (các tệp nguồn prototype đã được chỉnh sửa sau lần đóng gói cuối cùng), hoặc sau khi chỉnh sửa thủ công các tệp nguồn prototype.

**Quy trình thực hiện**:
1. Xác minh rằng prototype tồn tại tại `prototypes/{feature}/`
2. Chạy script đóng gói để rebuild `bundle.html` từ các tệp nguồn hiện tại
3. Khi thành công, cập nhật `bundleStatus` thành `"current"` trong tệp tiến độ
...
**Cú pháp**: `/planning-plugin:pp-design-system [--domain=b2b-admin|hotel-travel] [--query="context"]`
...
Khi bạn chạy `/planning-plugin:pp-spec`, **tác tử Analyst** bắt đầu bằng cách tự động quét dự án của bạn:
...
4. Các bước tiếp theo được đề xuất:
   - `/planning-plugin:pp-design-system --domain=...` — Xây dựng hệ thống thiết kế theo domain (khuyến nghị trước khi chạy design pipeline)
   - `/planning-plugin:pp-design {feature}` — Tạo UI DSL và wireframe, sau đó `/planning-plugin:pp-prototype {feature}` để tạo prototype React
   - `/planning-plugin:pp-review {feature}` — Đánh giá lại bất cứ lúc nào
   - Chỉnh sửa trực tiếp đặc tả ngôn ngữ làm việc và chạy `/planning-plugin:pp-translate {feature}` để đồng bộ
...
Skill `pp-sync-notion` đọc trực tiếp các tệp đặc tả và tạo trang cha + 3 trang con (Overview, Screens, Test Scenarios) cho mỗi ngôn ngữ dưới URL trang cha đã cấu hình. Mỗi trang con chứa nội dung của một tệp đặc tả, tránh giới hạn token đầu ra LLM với đặc tả lớn. Lưu URL trang vào tệp tiến độ để cập nhật trong tương lai. Được kích hoạt tự động sau khi hoàn thiện và dịch, hoặc thủ công qua `/planning-plugin:pp-sync-notion`.
...
Đọc UI DSL JSON và tạo dự án Vite + React 19 + TypeScript + TailwindCSS + shadcn/ui + React Router v7 + Lucide hoàn chỉnh trong `prototypes/{feature}/`, sau đó đóng gói thành một tệp `bundle.html` độc lập duy nhất (có thể mở qua `file://`). Bao gồm dữ liệu mock, routing trang và tất cả thành phần shadcn/ui được tham chiếu. Prototype là độc lập — không phụ thuộc vào dự án chính. Sử dụng mô hình Opus.

### Stitch Wireframe

**Vai trò**: Tạo wireframe trực quan từ UI DSL qua Google Stitch MCP

Đọc UI DSL JSON và chuyển đổi cây thành phần thành prompt ngôn ngữ tự nhiên cho Stitch, tạo các thiết kế wireframe trực quan. Trích xuất design token (màu sắc, typography, khoảng cách) và gợi ý ánh xạ thành phần shadcn/ui từ HTML/CSS được tạo ra. Đầu ra được sử dụng bởi Prototype Generator để nâng cao độ trung thực trực quan. Stage này là tùy chọn và yêu cầu cấu hình Stitch MCP (`claude mcp add stitch`). Sử dụng mô hình Opus.

## Cấu hình

Plugin sử dụng `.claude/planning-plugin.json` trong thư mục dự án của người dùng (được tạo bởi `/planning-plugin:pp-init`):
...
```
docs/specs/_shared/                        ← Màn hình layout chia sẻ (sidebar + header shell toàn ứng dụng)
├── en/
│   └── screens.md                         ← Định nghĩa màn hình layout (component Slot)
├── ui-dsl/                                ← DSL layout chia sẻ (được tính năng tham chiếu qua directive @layout:)
│   ├── manifest.json
│   └── screen-{layout-id}.json
└── stitch-wireframes/                     ← Wireframe layout chia sẻ (tùy chọn)
    └── ...
...
│   ├── screens.md                         ← Định nghĩa màn hình, Xử lý lỗi (có thể bao gồm directive @layout:)
│   └── test-scenarios.md                  ← Yêu cầu phi chức năng, Kịch bản kiểm thử
...
│   ├── manifest.json                      ← Chỉ mục màn hình + bản đồ điều hướng (layout chia sẻ có source: "_shared")
...
│   ├── DESIGN.md                          ← Tài liệu thiết kế định dạng Google (YAML token front-matter + 8 phần prose)
...
prototypes/{feature}/                  ← Prototype React (dự án Vite độc lập)
├── bundle.html                            ← Sản phẩm cuối cùng (HTML độc lập, mở qua file://)
├── package.json
├── src/
│   ├── App.tsx
...
- **Cung cấp mô tả tính năng chi tiết** — Bạn cung cấp càng nhiều ngữ cảnh trong lệnh `/planning-plugin:pp-spec` ban đầu, câu hỏi của Analyst sẽ càng chính xác. "Social login" thì được; "social login with Google and Apple for both web and mobile apps, replacing the current email-only signup" thì tốt hơn nhiều.
...
- **Chỉnh sửa thủ công được hoan nghênh** — Bạn có thể chỉnh sửa trực tiếp đặc tả ngôn ngữ làm việc bất cứ lúc nào. Sau khi chỉnh sửa, chạy `/planning-plugin:pp-translate feature-name` để đồng bộ bản dịch, và `/planning-plugin:pp-review feature-name` để kiểm tra lại chất lượng.

- **Sử dụng `--file` cho dịch có mục tiêu** — Nếu bạn chỉ thay đổi một tệp, sử dụng `/planning-plugin:pp-translate feature-name --file=screens` thay vì dịch lại toàn bộ đặc tả.

- **Kiểm tra trạng thái thường xuyên** — Sử dụng `/planning-plugin:pp-progress` (không tham số) để xem tổng quan tất cả đặc tả, đặc biệt khi làm việc trên nhiều tính năng cùng lúc.

- **Tiếp tục phiên làm việc** — Nếu bạn đóng Claude Code giữa chừng, plugin sẽ tự động phát hiện các đặc tả đang xử lý khi khởi động lại và thông báo cho bạn. Sử dụng `/planning-plugin:pp-progress` để xem bạn dừng ở đâu, sau đó `/planning-plugin:pp-spec` để tiếp tục.
...
- **Đánh giá lại sau thay đổi lớn** — Ngay cả sau khi hoàn thiện, bạn có thể đánh giá lại bất cứ lúc nào bằng `/planning-plugin:pp-review`. Điều này sẽ đổi trạng thái về `reviewing` để bạn có thể lặp lại thêm.
...
- **Thay đổi ngôn ngữ làm việc** — Có hai kịch bản:
  ...
  - *Đối với đặc tả hiện có*: Chạy `/planning-plugin:pp-migrate-language feature-name --to=vi`. Lệnh này chuyển source of truth sang bản dịch ngôn ngữ đích, đánh dấu tất cả bản dịch khác là cần đồng bộ, và giữ nguyên trạng thái đặc tả. Bản dịch ngôn ngữ đích phải tồn tại trước — chạy `/planning-plugin:pp-translate` trước nếu cần.
...
- **Wireframe Stitch lỗi thời**: Cảnh báo nếu tệp UI DSL được chỉnh sửa sau lần tạo wireframe Stitch cuối cùng (`design.stages.stitch.status: "stale"`)

### PostToolUse — `validate-spec-format.sh`

Chạy sau mỗi lệnh gọi công cụ `Write` hoặc `Edit`. Chỉ kích hoạt trên các tệp trong `docs/specs/` hoặc `prototypes/`:
...
- **Phát hiện bundle lỗi thời**: Nếu tệp nguồn prototype trong `prototypes/{feature}/src/` được chỉnh sửa và `bundleStatus` là `"current"`, tự động chuyển sang `"stale"`
- **Phát hiện Stitch lỗi thời**: Nếu tệp UI DSL trong `docs/specs/{feature}/ui-dsl/` được chỉnh sửa và `design.stages.stitch.status` là `"completed"`, tự động chuyển sang `"stale"`
...
- Tạo wireframe Stitch là tùy chọn và yêu cầu cấu hình Google Stitch MCP
- Layout chia sẻ: `_shared` là thư mục pseudo-feature dành riêng (`docs/specs/_shared/`) cho các màn hình layout toàn ứng dụng được chia sẻ giữa nhiều tính năng. Các tính năng tham chiếu layout chia sẻ qua directive `<!-- @layout: _shared/main-layout -->` trong `screens.md`. Prototype sao chép component layout chia sẻ cục bộ (copy-on-reference) để duy trì tính tự chứa
- Tự động tạo layout chia sẻ: Skill `pp-spec` tự động phát hiện pattern layout chia sẻ và tùy chọn tạo `_shared/en/screens.md` trong quá trình tạo bản nháp (Step 3.5). Luôn tạo trong thư mục `en/` bất kể `workingLanguage`
