...
- **Stitch Wireframe** — Google Stitch MCP를 통해 UI DSL에서 시각적 와이어프레임을 생성합니다
- **Prototype Generator** — UI DSL(+ Stitch 와이어프레임)에서 독립형 React 프로토타입을 생성합니다
...
## MCP 설정 (Notion & Stitch)

### Notion (Claude 내장 — OAuth)

Notion MCP 서버는 Claude에 내장되어 있으며 기본적으로 사용 가능합니다. OAuth를 통한 인증만 하면 바로 사용할 수 있습니다.

| 서버 | 사용처 |
|------|--------|
| `notion` | Notion Syncer 에이전트 (`/planning-plugin:pp-sync-notion`) |

사용 환경에서 Notion MCP 서버를 사용할 수 없는 경우, 수동으로 추가하십시오:
```
claude mcp add notion --transport http https://mcp.notion.com/mcp -s user
```

### Stitch (Google Stitch MCP — 선택)

Stitch MCP 서버는 API key header가 필요하므로 `plugin.json`에 **번들되지 않습니다**. 수동으로 설정해야 합니다:

1. [Google AI Studio](https://aistudio.google.com/apikey)에서 Google AI API key를 발급받습니다
2. Stitch MCP 서버를 추가합니다:
   ```
   claude mcp add stitch --transport http https://stitch.googleapis.com/mcp --header "X-Goog-Api-Key: YOUR_API_KEY" -s user
   ```
3. `/mcp`를 실행하여 설정을 확인합니다 — 서버 목록에 `stitch`가 표시되어야 합니다

> **참고**: Stitch MCP는 `/planning-plugin:pp-design`의 Stage 2에서만 사용됩니다. 설정하지 않으면 해당 단계가 자동으로 건너뛰어지며 와이어프레임 없이 파이프라인이 계속됩니다.
...
`.claude/planning-plugin.json`에 작업 언어, 지원 언어, Notion URL(선택)을 설정합니다. `/planning-plugin:pp-spec` 실행 전에 이 단계가 필요합니다 — pp-spec 스킬이 이 파일에서 설정을 읽습니다.
...
**구문**: `/planning-plugin:pp-spec "feature description"`
...
3. 템플릿을 기반으로 작업 언어 명세서가 3개 파일로 생성됩니다
4. 기능의 화면이 사이드바 + 헤더 쉘을 공유하는 경우: 공유 레이아웃 패턴을 감지하고 선택적으로 `docs/specs/_shared/en/screens.md`를 생성합니다 ([공유 레이아웃](#공유-레이아웃-_shared) 참조)
5. 나머지 지원 언어 번역이 병렬로 생성됩니다
6. Planner와 Tester가 순차적으로 검토 및 점수를 매깁니다
7. 피드백을 반영하고, 번역이 동기화되며, 반복하거나 최종화합니다
...
**구문**: `/planning-plugin:pp-review feature-name`
...
5. 이슈를 반영하고, 최종화를 선택하면 번역이 최종화 전에 동기화됩니다
...
**구문**: `/planning-plugin:pp-translate feature-name [--file=<name>]`
...
3. `--file=<name>`이 지정된 경우 해당 파일만 재번역합니다 (예: `--file=screens`로 `screens.md`만 번역)
4. 진행 파일의 동기화 타임스탬프를 업데이트합니다
5. `notionParentPageUrl`이 설정된 경우, 번역된 언어를 Notion에 자동으로 동기화합니다
6. Translator가 모호한 내용에 남긴 `<!-- NEEDS_REVIEW -->` 마커가 있으면 보고합니다

**예시**:
```
/planning-plugin:pp-translate social-login                    # 전체 동기화 (모든 파일)
/planning-plugin:pp-translate social-login --file=screens       # screens.md만 동기화
```
...
**구문**: `/planning-plugin:pp-progress [feature-name]`
...
Design Status, Design System 및 Notion Sync 섹션은 진행 파일에 해당 필드가 존재할 때만 표시됩니다.
...
**구문**: `/planning-plugin:pp-migrate-language feature-name --to=vi`
...
**구문**: `/planning-plugin:pp-sync-notion feature-name [--lang=xx]`
...
**구문**: `/planning-plugin:pp-design feature-name [--stage=dsl|stitch]`

**사용 시점**: 명세서 최종화 후 UI DSL과 시각적 와이어프레임을 생성할 때 사용합니다.

**동작 과정** (전체 파이프라인):
1. **Stage 1 — DSL 생성**: DSL Generator 에이전트가 `screens.md`와 `{feature}-spec.md`를 읽고, `docs/specs/{feature}/ui-dsl/`에 구조화된 UI DSL JSON 파일을 생성합니다 (화면 인덱스 + 내비게이션 맵이 포함된 `manifest.json`과 화면별 `screen-{id}.json`)
2. **Stage 2 — Stitch 와이어프레임** (선택): Stitch Wireframe 에이전트가 UI DSL을 읽고 Google Stitch MCP를 통해 시각적 와이어프레임을 생성하며, 디자인 토큰과 shadcn/ui 매핑 힌트를 `docs/specs/{feature}/stitch-wireframes/`에 추출합니다

기본 실행은 Stage 1→2를 수행한 후 리뷰 게이트에서 멈춥니다 — Stitch에서 와이어프레임을 검토한 후 프로토타입을 생성합니다. `--stage`를 사용하여 개별 단계를 독립적으로 실행할 수 있습니다.
...
**예시**:
```
/planning-plugin:pp-design social-login                    # 전체 파이프라인 (단계 1→2 + 리뷰 게이트)
/planning-plugin:pp-design social-login --stage=dsl        # DSL 생성만
/planning-plugin:pp-design social-login --stage=stitch     # Stitch 와이어프레임 생성만
```

> **참고**: Stage 2 (Stitch)는 선택 사항입니다. Stitch는 `claude mcp add stitch --transport http https://stitch.googleapis.com/mcp --header "X-Goog-Api-Key: <key>" -s user` 설정이 필요합니다. 프로토타입을 생성하려면, 와이어프레임 검토 후 `/planning-plugin:pp-prototype {feature}`를 실행하십시오.

**공유 레이아웃 (`_shared`)**: 여러 기능에서 공유하는 앱 전체 레이아웃 화면(사이드바 + 헤더 쉘)의 DSL을 생성하려면 feature 이름으로 `_shared`를 사용하십시오. 기능은 `screens.md`의 `<!-- @layout: _shared/main-layout -->` 디렉티브를 통해 공유 레이아웃을 참조합니다. 권장 워크플로우:

```
/planning-plugin:pp-design _shared                         # 공유 레이아웃 DSL 생성 (+ 선택적 Stitch)
/planning-plugin:pp-design social-login                    # 기능 DSL이 공유 레이아웃을 자동 참조
/planning-plugin:pp-prototype social-login                 # 프로토타입이 공유 레이아웃 컴포넌트를 로컬에 복사
```

> `_shared`는 스펙 라이프사이클 검증을 건너뜁니다 (progress 파일 불필요) — `docs/specs/_shared/en/screens.md`만 존재하면 됩니다. `@layout:` 디렉티브가 없는 기능은 기존과 동일하게 로컬 레이아웃 감지를 사용합니다 (하위 호환).

**공유 레이아웃 자동 생성**: `/planning-plugin:pp-spec` 실행 시, 플러그인이 Analyst의 사용자 흐름 답변에서 공유 레이아웃 패턴을 자동으로 감지합니다. 화면들이 영구적인 사이드바 + 헤더 쉘을 공유하는 경우:
- `_shared/en/screens.md`가 이미 존재하는 경우: `@layout:` 디렉티브가 자동으로 활성화되며, 기능의 화면 정의에서 쉘 컴포넌트가 제거됩니다.
- `_shared/en/screens.md`가 존재하지 않는 경우: 생성 여부를 확인합니다. 승인 시, `Slot` 컴포넌트가 포함된 공유 레이아웃 화면이 생성되고 기능의 `@layout:` 디렉티브가 활성화됩니다. 최종화 단계에서 `pp-design _shared`를 `pp-design {feature}` 전에 실행하도록 안내합니다.
- 패턴이 감지되지 않거나 거절한 경우: 기능이 기존과 동일하게 로컬 레이아웃 감지를 사용합니다.

---

### `/planning-plugin:pp-prototype`

**구문**: `/planning-plugin:pp-prototype feature-name`

**사용 시점**: Stitch 와이어프레임을 검토한 후 (또는 Stitch를 건너뛴 경우 DSL 생성 후) React 프로토타입을 생성할 때 사용합니다.

**동작 과정**:
1. 기능의 UI DSL이 공유 레이아웃을 참조하는 경우(`source: "_shared"`), `docs/specs/_shared/ui-dsl/manifest.json`이 존재하는지 검증합니다
2. Stitch 와이어프레임이 존재하는 경우, 생성 전에 Stitch에서 최신 변경 사항을 동기화할지 안내합니다
3. Prototype Generator 에이전트가 UI DSL(및 Stitch 출력물이 있는 경우 이를 참조)을 읽고, `prototypes/{feature}/`에 독립형 Vite + React 19 + TypeScript + TailwindCSS + shadcn/ui + React Router v7 + Lucide 프로젝트를 생성합니다
4. 단일 독립형 `bundle.html`(`file://`로 열 수 있음)로 번들링합니다

> 공유 레이아웃 컴포넌트는 프로토타입의 독립성을 유지하기 위해 프로토타입 내부에 로컬로 복사됩니다 (copy-on-reference).
...
**구문**: `/planning-plugin:pp-sync-stitch feature-name [--screen=screen-id]`

**사용 시점**: Stitch 웹사이트에서 와이어프레임을 편집한 후, 업데이트된 HTML/PNG/디자인 토큰을 로컬 프로젝트에 가져올 때 사용합니다. DSL에서 와이어프레임을 재생성하는 `pp-design --stage=stitch`와 달리, `pp-sync-stitch`는 기존 Stitch 스크린에서 콘텐츠를 다시 가져옵니다.
...
| 시나리오 | 명령어 |
|----------|--------|
| Stitch 웹사이트에서 와이어프레임을 편집한 경우 | `/planning-plugin:pp-sync-stitch {feature}` |
| UI DSL이 변경된 경우 (스펙 화면 변경) | `/planning-plugin:pp-design {feature} --stage=stitch` |
| 특정 스크린만 동기화가 필요한 경우 | `/planning-plugin:pp-sync-stitch {feature} --screen=screen-id` |
...
**구문**: `/planning-plugin:pp-bundle feature-name`

**사용 시점**: `bundleStatus`가 `"stale"`인 경우 (마지막 번들 이후 프로토타입 소스 파일이 편집된 경우), 또는 프로토타입 소스 파일을 수동으로 수정한 후 사용합니다.

**동작 과정**:
1. `prototypes/{feature}/`에 프로토타입이 존재하는지 검증합니다
2. 번들 스크립트를 실행하여 현재 소스 파일에서 `bundle.html`을 재빌드합니다
3. 성공 시 진행 파일의 `bundleStatus`를 `"current"`로 업데이트합니다
...
**구문**: `/planning-plugin:pp-design-system [--domain=b2b-admin|hotel-travel] [--query="context"]`
...
`/planning-plugin:pp-spec`을 실행하면 **Analyst 에이전트**가 먼저 프로젝트를 자동으로 스캔합니다:
...
플러그인이 답변 내용을 기반으로 3개 템플릿 파일을 채웁니다 (선택적 읽기를 위해 분리됨):
...
- **어느 한 점수라도 < 8이고 3라운드 미만인 경우**: 최종화를 제안하지 않습니다. "추가 리뷰 라운드를 권장합니다."
- **3라운드 완료 후에도 어느 한 점수가 < 8인 경우**: "3라운드 후 남아 있는 미결 사항은 다음과 같습니다. 현 상태로 최종화하시겠습니까?"
...
4. 다음 단계가 제안됩니다:
   - `/planning-plugin:pp-design-system --domain=...` — 도메인별 디자인 시스템 구축 (디자인 파이프라인 실행 전 권장)
   - `/planning-plugin:pp-design {feature}` — UI DSL, 와이어프레임 생성 후 `/planning-plugin:pp-prototype {feature}`로 React 프로토타입 생성
   - `/planning-plugin:pp-review {feature}` — 언제든지 재검토
   - 작업 언어 명세서를 직접 편집 후 `/planning-plugin:pp-translate {feature}`로 동기화
...
`pp-sync-notion` 스킬이 명세서 파일을 직접 읽고 언어당 부모 페이지 + 자식 페이지 3개(Overview, Screens, Test Scenarios)를 설정된 상위 페이지 URL 하위에 생성합니다. 각 자식 페이지가 하나의 명세서 파일 내용을 담으므로 대용량 스펙에서 LLM 출력 토큰 한계를 회피합니다. 향후 업데이트를 위해 페이지 URL을 진행 파일에 저장합니다. 최종화 및 번역 후 자동으로 트리거되거나, `/planning-plugin:pp-sync-notion`을 통해 수동으로 실행할 수 있습니다.

### DSL Generator

**역할**: 화면 정의를 구조화된 UI DSL JSON으로 변환

최종화된 명세서의 `screens.md`와 `{feature}-spec.md`를 읽고, `docs/specs/{feature}/ui-dsl/`에 구조화된 JSON 파일을 생성합니다. 출력물에는 `manifest.json`(화면 인덱스 + 내비게이션 맵)과 화면별 `screen-{id}.json`이 포함됩니다. shadcn/ui 컴포넌트 어휘만을 사용합니다. Opus 모델을 사용합니다.

### Prototype Generator

**역할**: UI DSL에서 독립형 React 프로토타입 생성

UI DSL JSON을 읽고, `prototypes/{feature}/`에 완전한 Vite + React 19 + TypeScript + TailwindCSS + shadcn/ui + React Router v7 + Lucide 프로젝트를 생성한 후, 단일 독립형 `bundle.html`(`file://`로 열 수 있음)로 번들링합니다. 목업 데이터, 페이지 라우팅, 참조된 모든 shadcn/ui 컴포넌트를 포함합니다. 프로토타입은 독립형으로, 메인 프로젝트에 대한 의존성이 없습니다. Opus 모델을 사용합니다.

### Stitch Wireframe

**역할**: Google Stitch MCP를 통해 UI DSL에서 시각적 와이어프레임 생성

UI DSL JSON을 읽고, 컴포넌트 트리를 Stitch용 자연어 프롬프트로 변환하여 시각적 와이어프레임 디자인을 생성합니다. 생성된 HTML/CSS에서 디자인 토큰(색상, 타이포그래피, 간격)과 shadcn/ui 컴포넌트 매핑 힌트를 추출합니다. 출력물은 Prototype Generator가 시각적 품질을 향상시키는 데 활용합니다. 이 단계는 선택 사항이며 Stitch MCP 설정(`claude mcp add stitch`)이 필요합니다. Opus 모델을 사용합니다.

## 설정

플러그인은 사용자 프로젝트 디렉토리의 `.claude/planning-plugin.json`을 사용하여 설정합니다 (`/planning-plugin:pp-init`으로 생성):
...
```
docs/specs/_shared/                        ← 공유 레이아웃 화면 (앱 전체 사이드바 + 헤더 쉘)
├── en/
│   └── screens.md                         ← 레이아웃 화면 정의 (Slot 컴포넌트)
├── ui-dsl/                                ← 공유 레이아웃 DSL (기능에서 @layout: 디렉티브로 참조)
│   ├── manifest.json
│   └── screen-{layout-id}.json
└── stitch-wireframes/                     ← 공유 레이아웃 와이어프레임 (선택)
    └── ...

docs/specs/{feature}/
├── {workingLanguage}/                     ← Source of truth (작업 언어)
│   ├── {feature}-spec.md                  ← 인덱스: 개요, 사용자 스토리, 기능 요구사항, 미결 사항, 리뷰 이력
│   ├── screens.md                         ← 화면 정의, 오류 처리 (@layout: 디렉티브 포함 가능)
│   └── test-scenarios.md                  ← 비기능 요구사항, 테스트 시나리오
...
│   ├── manifest.json                      ← 화면 인덱스 + 내비게이션 맵 (공유 레이아웃은 source: "_shared" 포함)
...
prototypes/{feature}/                  ← React 프로토타입 (독립형 Vite 프로젝트)
├── bundle.html                            ← 최종 산출물 (단일 독립형 HTML, file://로 열기 가능)
├── package.json
├── src/
│   ├── App.tsx
...
- **상세한 기능 설명을 제공하십시오** — 초기 `/planning-plugin:pp-spec` 명령에 더 많은 컨텍스트를 제공할수록 Analyst의 질문이 더 정확해집니다. "Social login"도 괜찮지만, "social login with Google and Apple for both web and mobile apps, replacing the current email-only signup"이 훨씬 좋습니다.

- **TBD 항목을 영원히 방치하지 마십시오** — TBD 마커는 진행을 가능하게 하지만, 최종화 전에 반드시 돌아와서 해결하십시오. Planner와 Tester가 미해결 TBD를 이슈로 지적합니다.

- **수동 편집을 환영합니다** — 언제든지 작업 언어 명세서를 직접 편집하실 수 있습니다. 편집 후 `/planning-plugin:pp-translate feature-name`으로 번역을 동기화하고, `/planning-plugin:pp-review feature-name`으로 품질을 재확인하십시오.

- **타겟 번역을 위해 `--file`을 사용하십시오** — 하나의 파일만 변경한 경우, 전체 명세서를 재번역하는 대신 `/planning-plugin:pp-translate feature-name --file=screens`를 사용하십시오.

- **정기적으로 상태를 확인하십시오** — `/planning-plugin:pp-progress` (인수 없이)를 사용하여 모든 명세서를 한눈에 확인하십시오. 특히 여러 기능을 동시에 작업할 때 유용합니다.

- **세션 재개** — 워크플로우 진행 중에 Claude Code를 종료하면, 플러그인이 재시작 시 진행 중인 명세서를 자동으로 감지하여 알려줍니다. `/planning-plugin:pp-progress`로 중단 지점을 확인한 후 `/planning-plugin:pp-spec`으로 재개하십시오.

- **완벽한 점수를 쫓지 마십시오** — 3라운드 후 점수가 정체되면, 플러그인이 미결 사항과 함께 최종화를 제안합니다. 이것이 올바른 선택인 경우가 많습니다 — 미결 사항이 문서화된 최종 명세서가 끝없이 리뷰되는 초안보다 유용합니다.

- **주요 변경 후 리뷰하십시오** — 최종화 이후에도 `/planning-plugin:pp-review`로 언제든지 재검토할 수 있습니다. 이 경우 상태가 `reviewing`으로 변경되어 추가 반복이 가능합니다.
...
- **작업 언어를 변경하려면** — 두 가지 시나리오가 있습니다:
  ...
  - *기존 명세서의 경우*: `/planning-plugin:pp-migrate-language feature-name --to=vi`를 실행하십시오. 대상 언어 번역을 새 source of truth로 전환하고, 나머지 번역을 동기화 필요 상태로 표시하며, 명세서의 상태(status)는 유지됩니다. 대상 언어의 번역 파일이 먼저 존재해야 합니다 — 필요한 경우 `/planning-plugin:pp-translate`를 먼저 실행하십시오.
...
- **오래된 Stitch 와이어프레임**: 마지막 Stitch 와이어프레임 생성 이후 UI DSL 파일이 편집된 경우 경고합니다 (`design.stages.stitch.status: "stale"`)

### PostToolUse — `validate-spec-format.sh`

모든 `Write` 또는 `Edit` 도구 호출 이후 실행됩니다. `docs/specs/` 또는 `prototypes/` 하위 파일에서만 활성화됩니다:
...
- **번들 오래된 상태 감지**: `prototypes/{feature}/src/` 하위의 프로토타입 소스 파일이 편집되고 `bundleStatus`가 `"current"`인 경우, 자동으로 `"stale"`로 전환합니다
- **Stitch 오래된 상태 감지**: `docs/specs/{feature}/ui-dsl/` 하위의 UI DSL 파일이 편집되고 `design.stages.stitch.status`가 `"completed"`인 경우, 자동으로 `"stale"`로 전환합니다

## 규칙

- 기술 용어 (API, endpoint, schema, CRUD)는 모든 번역에서 영어로 유지됩니다
- 모든 에이전트 리뷰는 작업 언어 명세서 디렉토리만을 대상으로 합니다
- 명세서는 언어 디렉토리당 3개 파일로 분리됩니다 — `{feature}-spec.md`이 인덱스 파일이며, 상세 파일(`screens.md`, `test-scenarios.md`)이 나머지를 담당합니다
...
- Stitch 와이어프레임 생성은 선택 사항이며 Google Stitch MCP 설정이 필요합니다
- 공유 레이아웃: `_shared`는 여러 기능에서 공유하는 앱 전체 레이아웃 화면을 위한 예약된 pseudo-feature 디렉토리(`docs/specs/_shared/`)입니다. 기능은 `screens.md`의 `<!-- @layout: _shared/main-layout -->` 디렉티브를 통해 공유 레이아웃을 참조합니다. 프로토타입은 독립성을 유지하기 위해 공유 레이아웃 컴포넌트를 로컬에 복사합니다 (copy-on-reference)
- 공유 레이아웃 자동 생성: `pp-spec` 스킬이 초안 생성 시 (Step 3.5) 공유 레이아웃 패턴을 자동으로 감지하고 선택적으로 `_shared/en/screens.md`를 생성합니다. `workingLanguage`와 무관하게 항상 `en/` 디렉토리에 생성됩니다
