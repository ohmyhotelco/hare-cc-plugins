...
- **오래된 Stitch 와이어프레임**: 마지막 Stitch 와이어프레임 생성 이후 UI DSL 파일이 편집된 경우 경고합니다 (`stitch.status: "stale"`)
...
- **Stitch 오래된 상태 감지**: `docs/specs/{feature}/ui-dsl/` 하위의 UI DSL 파일이 편집되고 `stitch.status`가 `"completed"`인 경우, 자동으로 `"stale"`로 전환합니다
