...
## MCP 설정 (Figma, Notion & Stitch)

### Figma & Notion (번들 — OAuth)
...
> **참고**: Stitch MCP는 `/planning-plugin:design`의 Stage 1.5에서만 사용됩니다. 설정하지 않으면 해당 단계가 자동으로 건너뛰어지며 와이어프레임 없이 파이프라인이 계속됩니다.
...
**구문**: `/planning-plugin:design feature-name [--stage=dsl|stitch|prototype|figma]`

**사용 시점**: 명세서 최종화 후 UI DSL, 시각적 와이어프레임, React 프로토타입, 그리고 선택적으로 Figma 디자인을 생성할 때 사용합니다.
...
2. **Stage 1.5 — Stitch 와이어프레임** (선택): Stitch Wireframe 에이전트가 UI DSL을 읽고 Google Stitch MCP를 통해 시각적 와이어프레임을 생성하며, 디자인 토큰과 shadcn/ui 매핑 힌트를 `docs/specs/{feature}/stitch-wireframes/`에 추출합니다
3. **Stage 2 — 프로토타입 생성**: Prototype Generator 에이전트가 UI DSL(및 Stitch 출력물이 있는 경우 이를 참조)을 읽고, `src/prototypes/{feature}/`에 독립형 Vite + React 19 + TypeScript + TailwindCSS + shadcn/ui + React Router v7 + Lucide 프로젝트를 생성한 후, 단일 독립형 `bundle.html`로 번들링합니다
4. **Stage 3 — Figma 생성** (선택): Figma Designer 에이전트가 React 프로토타입 코드를 읽고, `generate_figma_design` MCP 도구를 통해 Figma 레이어로 변환합니다

단계는 순차적으로 실행됩니다 (1→1.5→2→3). `--stage`를 사용하여 개별 단계를 독립적으로 실행할 수 있습니다.
...
**예시**:
```
/planning-plugin:design social-login                    # 전체 파이프라인 (단계 1→1.5→2→3)
...
> **참고**: Stage 1.5 (Stitch)와 Stage 3 (Figma)은 선택 사항입니다. Stitch는 `claude mcp add stitch --transport http https://stitch.googleapis.com/mcp --header "X-Goog-Api-Key: <key>" -s user` 설정이 필요합니다. Figma는 Figma MCP 설정이 필요합니다.
