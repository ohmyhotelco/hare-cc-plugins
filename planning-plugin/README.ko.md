...
**동작 과정**:
1. Stitch 와이어프레임이 존재하는 경우, 생성 전에 Stitch에서 최신 변경 사항을 동기화할지 안내합니다
2. Prototype Generator 에이전트가 UI DSL(및 Stitch 출력물이 있는 경우 이를 참조)을 읽고, `src/prototypes/{feature}/`에 독립형 Vite + React 19 + TypeScript + TailwindCSS + shadcn/ui + React Router v7 + Lucide 프로젝트를 생성합니다
3. 단일 독립형 `bundle.html`(`file://`로 열 수 있음)로 번들링합니다
