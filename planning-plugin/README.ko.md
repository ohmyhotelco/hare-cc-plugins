...
| 서버 | 사용처 |
|------|--------|
| `notion` | Notion Syncer 에이전트 (`/planning-plugin:sync-notion`) |
...
> **참고**: Stitch MCP는 `/planning-plugin:design`의 Stage 2에서만 사용됩니다. 설정하지 않으면 해당 단계가 자동으로 건너뛰어지며 와이어프레임 없이 파이프라인이 계속됩니다.
...
/planning-plugin:translate social-login --file=screens       # screens.md만 동기화
```
...
**구문**: `/planning-plugin:design feature-name [--stage=dsl|stitch]`
...
**예시**:
```
/planning-plugin:design social-login                    # 전체 파이프라인 (단계 1→2 + 리뷰 게이트)
/planning-plugin:design social-login --stage=dsl        # DSL 생성만
/planning-plugin:design social-login --stage=stitch     # Stitch 와이어프레임 생성만
```

> **참고**: Stage 2 (Stitch)는 선택 사항입니다. Stitch는 `claude mcp add stitch --transport http https://stitch.googleapis.com/mcp --header "X-Goog-Api-Key: <key>" -s user` 설정이 필요합니다. 프로토타입을 생성하려면, 와이어프레임 검토 후 `/planning-plugin:prototype {feature}`를 실행하십시오.
...
```
/planning-plugin:design _shared                         # 공유 레이아웃 DSL 생성 (+ 선택적 Stitch)
/planning-plugin:design social-login                    # 기능 DSL이 공유 레이아웃을 자동 참조
/planning-plugin:prototype social-login                 # 프로토타입이 공유 레이아웃 컴포넌트를 로컬에 복사
```
...
**공유 레이아웃 자동 생성**: `/planning-plugin:spec` 실행 시, 플러그인이 Analyst의 사용자 흐름 답변에서 공유 레이아웃 패턴을 자동으로 감지합니다. 화면들이 영구적인 사이드바 + 헤더 쉘을 공유하는 경우:
- `_shared/en/screens.md`가 이미 존재하는 경우: `@layout:` 디렉티브가 자동으로 활성화되며, 기능의 화면 정의에서 쉘 컴포넌트가 제거됩니다.
- `_shared/en/screens.md`가 존재하지 않는 경우: 생성 여부를 확인합니다. 승인 시, `Slot` 컴포넌트가 포함된 공유 레이아웃 화면이 생성되고 기능의 `@layout:` 디렉티브가 활성화됩니다. 최종화 단계에서 `design _shared`를 `design {feature}` 전에 실행하도록 안내합니다.
...
**구문**: `/planning-plugin:prototype feature-name`
...
4. 다음 단계가 제안됩니다:
   ...
   - `/planning-plugin:design {feature}` — UI DSL, 와이어프레임 생성 후 `/planning-plugin:prototype {feature}`로 React 프로토타입 생성
...
- **타겟 번역을 위해 `--file`을 사용하십시오** — 하나의 파일만 변경한 경우, 전체 명세서를 재번역하는 대신 `/planning-plugin:translate feature-name --file=screens`를 사용하십시오.
...
- 공유 레이아웃 자동 생성: `spec` 스킬이 초안 생성 시 (Step 3.5) 공유 레이아웃 패턴을 자동으로 감지하고 선택적으로 `_shared/en/screens.md`를 생성합니다. `workingLanguage`와 무관하게 항상 `en/` 디렉토리에 생성됩니다
