...
**구문**: `/planning-plugin:design feature-name [--stage=dsl|stitch|prototype]`

**사용 시점**: 명세서 최종화 후 UI DSL, 시각적 와이어프레임, React 프로토타입을 생성할 때 사용합니다.
...
3. **Stage 3 — 프로토타입 생성**: Prototype Generator 에이전트가 UI DSL(및 Stitch 출력물이 있는 경우 이를 참조)을 읽고, `src/prototypes/{feature}/`에 독립형 Vite + React 19 + TypeScript + TailwindCSS + shadcn/ui + React Router v7 + Lucide 프로젝트를 생성한 후, 단일 독립형 `bundle.html`로 번들링합니다
...
> **참고**: Stage 2 (Stitch)는 선택 사항입니다. Stitch는 `claude mcp add stitch --transport http https://stitch.googleapis.com/mcp --header "X-Goog-Api-Key: <key>" -s user` 설정이 필요합니다.
...
4. 다음 단계가 제안됩니다:
   ...
   - `/planning-plugin:design {feature}` — UI DSL, React 프로토타입 생성
