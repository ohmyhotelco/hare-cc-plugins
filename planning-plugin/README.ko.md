...
3. Prototype Generator 에이전트가 UI DSL(및 Stitch 출력물이 있는 경우 이를 참조)을 읽고, `src/prototypes/{feature}/`에 독립형 Vite + React 19 + TypeScript + TailwindCSS + shadcn/ui + React Router v7 + Lucide 프로젝트를 생성합니다
...
**동작 과정**:
1. `src/prototypes/{feature}/`에 프로토타입이 존재하는지 검증합니다
...
UI DSL JSON을 읽고, `src/prototypes/{feature}/`에 완전한 Vite + React 19 + TypeScript + TailwindCSS + shadcn/ui + React Router v7 + Lucide 프로젝트를 생성한 후, 단일 독립형 `bundle.html`(`file://`로 열 수 있음)로 번들링합니다. 목업 데이터, 페이지 라우팅, 참조된 모든 shadcn/ui 컴포넌트를 포함합니다. 프로토타입은 독립형으로, 메인 프로젝트에 대한 의존성이 없습니다. Opus 모델을 사용합니다.
