...
이 플러그인은 하나의 HTTP MCP 서버를 번들로 포함합니다 (`plugin.json`에 정의됨):

| 서버 | URL | 사용처 |
|------|-----|--------|
| `notion` | `https://mcp.notion.com/mcp` | Notion Syncer 에이전트 (`/planning-plugin:sync-notion`) |

설치 시 이 서버가 자동으로 등록되므로 수동으로 `claude mcp add`를 실행할 필요가 없습니다.
...
> - Notion 인증은 `/planning-plugin:sync-notion`에 필요합니다.
