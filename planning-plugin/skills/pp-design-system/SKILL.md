...
2. If it does not exist, inform the user:
   > ⚠️ Planning plugin is not initialized. Run `/planning-plugin:pp-init` first to set up project configuration.
...
> **Domain**: {domain_label}
> **Generated**: {timestamp}
> **Generator**: planning-plugin:pp-design-system
...
_Generated by planning-plugin:pp-design-system at {timestamp}_
```
...
> 2. **Spec → Design**: Run `/planning-plugin:pp-design {feature}` — the DSL generator will reference `design-system/pages/components.md` for component selection
>
