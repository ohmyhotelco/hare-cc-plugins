...
### [Frontend Migration Plugin](./frontend-migration-plugin/) `v1.3.0`
...
**Pipeline**: `fm-init` → `fm-analyze` → `fm-extract` → (`fm-style-spec` → `fm-plan` → `fm-gen` → `fm-verify` → (`fm-cascade`) → `fm-e2e` → `fm-parity` → `fm-route`) ↔ `fm-fix`
